...
in the unified comparison (per task instructions #26, #51: "must directly
...
real data confirms `max` is this project's actual convention."), NOT the
mean-of-three-flutes convention that Paper A's own text uses for its own
(non-reused) EM-based label scheme. See PAPER_SPEC.md for how this
baseline's Original-Protocol (Protocol A) EM-based labels differ from
this Unified-Protocol (Protocol B) convention.

Only used for Protocol B (Unified). Protocol A (original-paper sanity
reproduction) uses Paper A's own EM-derived fixed pass-index ranges
...
os.environ.setdefault(
    "FGDS_RUN_DIR",
    str(Path(tempfile.gettempdir()) / "multi_source_attention_dcpsr_base_import_cache"),
)
