...
B12_SEARCH = {
    "eta": [0.75, 0.80, 0.85, 0.90, 0.95],
    "fine_weight": [0.00, 0.05, 0.10, 0.20, 0.30],
    "temperature": [0.80, 1.00, 1.20, 1.50, 2.00],
    "mid_floor": [0.00, 0.01, 0.02],
    "early_tau": [0.28, 0.32, 0.36, 0.40, 0.45],
    "late_tau": [0.55, 0.60, 0.65, 0.70, 0.75],
    "order_blend": [0.00, 0.03, 0.05, 0.08, 0.10, 0.15],
    "prior_sigma": [0.22, 0.28, 0.35, 0.45],
}
...
def b12_stageaware_score(metrics: dict[str, Any], n: int) -> float:
...
def random_fusion_params(rng: random.Random, q_val: dict[str, float]) -> dict[str, float]:
    poor_q = (q_val.get("Spearman", 0) < 0.25) or (q_val.get("Pearson", 0) < 0.25) or (q_val.get("q-R2", 0) < 0)
    good_q = (q_val.get("Spearman", 0) >= 0.50) and (q_val.get("Pearson", 0) >= 0.50)
    space = {k: list(v) for k, v in B12_SEARCH.items()}
    if poor_q:
        space["fine_weight"] = [0.00, 0.05, 0.10]
        space["eta"] = [0.85, 0.90, 0.95]
        space["prior_sigma"] = [0.35, 0.45]
    elif good_q:
        space["fine_weight"] = [0.00, 0.05, 0.10, 0.20, 0.30]
        space["eta"] = [0.75, 0.80, 0.85, 0.90]
...
def search_b12_params_stageaware(pred_val_raw: pd.DataFrame, task: dict[str, Any], seed: int, q_val: dict[str, float]) -> tuple[dict[str, float], float, dict[str, Any], list[dict[str, Any]]]:
    ...
    def eval_params(params: dict[str, float], phase: str) -> None:
        ...
        score = b12_stageaware_score(m, len(pred))
        evaluated.append({"Split_type": task["Split_type"], "Task": task["Task"], "Seed": seed, "Phase": phase, "Score": score, **params, **m})

    for _ in range(200):
        eval_params(random_fusion_params(rng, q_val), "random200")
    top = sorted(evaluated, key=lambda r: r["Score"], reverse=True)[:20]
    for row in top:
        base = {k: row[k] for k in B12_SEARCH}
        for eta in sorted(set([base["eta"], min(0.95, round(base["eta"] + 0.03, 2)), max(0.75, round(base["eta"] - 0.03, 2))])):
            for et in sorted(set([base["early_tau"], min(0.45, round(base["early_tau"] + 0.02, 2)), max(0.28, round(base["early_tau"] - 0.02, 2))])):
                for lt in sorted(set([base["late_tau"], min(0.75, round(base["late_tau"] + 0.03, 2)), max(0.55, round(base["late_tau"] - 0.03, 2))])):
                    for ob in sorted(set([base["order_blend"], min(0.15, round(base["order_blend"] + 0.02, 2)), max(0.00, round(base["order_blend"] - 0.02, 2))])):
                        p = dict(base)
                        p.update({"eta": eta, "early_tau": et, "late_tau": lt, "order_blend": ob})
                        eval_params(p, "local_top20")
    best = max(evaluated, key=lambda r: r["Score"])
    params = {k: best[k] for k in B12_SEARCH}
    metrics = {k: best[k] for k in ["Acc", "Macro-F1", "Balanced-Acc", "E-F1", "M-F1", "L-F1", "E-Rec", "M-Rec", "L-Rec", "M-Pre", "M→E", "M→L", "E→M", "L→M", "Rev", "Jump", "Smooth", "Predicted_class_count", "Early_recall", "Middle_recall", "Late_recall"]}
...
def main() -> None:
    ...
    with open(OUT_DIR / "NASA_experiment_config_stageaware.json", "w", encoding="utf-8") as f:
        json.dump({"RUN_SEEDS": RUN_SEEDS, "B12_SEARCH": B12_SEARCH, "fixed_tasks": FIXED_TASKS, "max_epochs": EPOCHS, "patience": PATIENCE}, f, ensure_ascii=False, indent=2)
    ...
    for seed in RUN_SEEDS:
        for task in original_tasks + balanced_tasks:
    ...
    save_csv(pd.DataFrame(all_diag), OUT_DIR / "NASA_experiment_diagnostics_stageaware.csv")
    save_csv(pd.DataFrame(all_fusion), OUT_DIR / "NASA_B12_best_fusion_params_stageaware.csv")
    save_csv(pd.DataFrame(all_fusion_logs), OUT_DIR / "NASA_B12_fusion_search_log.csv")
    ...
    for split_type in ["original", "stagebalanced"]:
        sub = mean_df[mean_df["Split_type"] == split_type]
        if len(sub) == 0:
            continue
        b12 = sub[sub["Method"] == "B12"].iloc[0]
        best_macro = sub.loc[sub["Macro-F1_mean"].idxmax()]
        print(f"{split_type}: best Macro-F1 method = {best_macro['Method']} ({best_macro['Macr o-F1_mean']:.4f})")
