# -*- coding: utf-8 -*-
"""
D2/D3 transfer-task training for DP2Net-adapted.

Reuses baselines/dp2net/train.py::run_protocol_b(variant="B-D1", ...)
UNMODIFIED, monkeypatching the same D2/D3 condition split (condition_split.py)
and train.OUT_ROOT (scratch path under this stage's own tree, never
touching outputs/dp2net/unified_protocol_B-D1/). variant stays "B-D1"
(pooled-source protocol) for D2/D3 too, per task instruction #17 -- only
which two conditions are pooled as source vs. held out as target changes.
No S/G/F architecture or hyperparameter (CFG) change.
"""
...
METHOD_DIR = REPO_ROOT / "baselines" / "dp2net"
...
def run(task: str, force: bool = False):
    ...
    import train as T  # baselines/dp2net/train.py
    ...
    t0 = time.time()
    metric_row = T.run_protocol_b(variant="B-D1", device=device, seed=TRAIN_SEED)
    t1 = time.time()

    native_dir = scratch_root / "unified_protocol_B-D1" / f"seed{TRAIN_SEED}"
    import pandas as pd
    pred_df = pd.read_csv(native_dir / "predictions.csv")
    ...
    ckpt_src = native_dir / "checkpoint_final.pth"
    if ckpt_src.exists():
