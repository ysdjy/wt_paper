# -*- coding: utf-8 -*-
"""
D2/D3 transfer-task training for Multi-source Attention.

Reuses baselines/multi_source_attention/train.py::run_protocol_b()
UNMODIFIED (imported as a module, never edited) with two monkeypatches
applied before the call:
  1. dcpsr_base.split_grouped_lifecycle -> parameterized D2/D3 split
     (condition_split.py; same logic as the D1 split, different conditions)
  2. train.OUT_ROOT -> a scratch path under this stage's own
     transfer_tasks/ tree, so baselines/multi_source_attention's own D1
     `outputs/multi_source_attention/unified_protocol/` result is never
     touched.
No architecture, CWT parameter, or hyperparameter (PROTO_B_CFG) is
changed -- only which two conditions are pooled as source vs. held out as
target.
"""
...
METHOD_DIR = REPO_ROOT / "baselines" / "multi_source_attention"
...
METHOD_ID = "multi_source_attention"
...
def run(task: str, force: bool = False):
    ...
    import train as T  # baselines/multi_source_attention/train.py
    ...
    t0 = time.time()
    metric_row = T.run_protocol_b(device)
    t1 = time.time()
